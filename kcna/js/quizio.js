...
async function loadQuizioData(){
  const el=document.getElementById('quizio-cards');
  el.innerHTML='<div class="empty" style="grid-column:1/-1"><div class="empty-icon">⏳</div><p>Loading exams…</p></div>';
  try{
    const res=await fetch('./kcna%20quizios/kcna_all_questions.json');
    if(!res.ok)throw new Error('HTTP '+res.status);
    QUIZIO_DATA=await res.json();
    renderQuizioExams(qzFilter);
  }catch(e){
...
function renderQuizioExams(filter){
  qzFilter=filter;
  ...
  if(!QUIZIO_DATA){loadQuizioData();return;}
  let exams=QUIZIO_DATA.exams;
  if(filter!=='all')exams=exams.filter(e=>e.exam_type===filter);
  const counts={STUDY:0,PREP:0,MOCK:0};
  QUIZIO_DATA.exams.forEach(e=>counts[e.exam_type]=(counts[e.exam_type]||0)+1);
  // Update tab counts
  document.querySelectorAll('.qz-tab').forEach(t=>{
    const tf=t.textContent.trim().split(' ')[0].toUpperCase();
    if(tf==='ALL')t.textContent=`All (${QUIZIO_DATA.exams.length})`;
    else if(counts[tf]!==undefined)t.textContent=`${QZ_TYPE_LABEL[tf]||tf} (${counts[tf]})`;
  });
  document.getElementById('quizio-cards').innerHTML=exams.map(e=>{
    ...
    return `<div class="exam-card">
      ...
      <button class="btn btn-primary btn-sm" onclick="startQuizioExam('${e.exam_code}')">▶ Start</button>
    </div>`;
  }).join('');
}

function startQuizioExam(examCode){
  if(!QUIZIO_DATA)return;
  qzExam=QUIZIO_DATA.exams.find(e=>e.exam_code===examCode);
  ...
  qzTimer=setInterval(()=>{
    ...
    if(rem<=0){clearInterval(qzTimer);finishQuizioExam();return;}
    const m=Math.floor(rem/60),s=rem%60;
    timerEl.textContent=`⏱ ${m}:${s.toString().padStart(2,'0')}`;
    timerEl.style.color=rem<=60?'var(--hard)':'var(--muted)';
  },1000);
  document.getElementById('quizio-browser').style.display='none';
  document.getElementById('quizio-results').style.display='none';
  document.getElementById('quizio-active').style.display='block';
  ...
  if(isMulti){
    document.getElementById('qz-options').innerHTML=`<div class="multi-check">${
      opts.map((opt,i)=>{
        ...
        const checkMark=submitted
          ...
          :(selected.includes(i)?'✓':'');
        const clickHandler=submitted?'':` onclick="toggleQuizioAnswer(${i})"`;
    ...
    document.getElementById('qz-options').innerHTML=`<div class="options">${
      opts.map((opt,i)=>{
        ...
        const clickHandler=submitted?'':` onclick="selectQuizioAnswer(${i})"`;
        return `<div class="${cls}"${clickHandler}>
...
function finishQuizioExam(){
  ...
  document.getElementById('quizio-results').style.display='block';
...
function retryQuizioExam(){if(qzLastExamCode)startQuizioExam(qzLastExamCode);}
